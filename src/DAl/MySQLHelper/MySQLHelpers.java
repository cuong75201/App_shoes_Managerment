...
//       xây dựng phần trường (fields) cần thiết cho câu lệnh INSERT
    ...
    // SELECT
    sqlBuilder.append("SELECT ").append(queryParams.get("SELECT")).append(" ");
    
    // FROM
    if (!queryParams.get("TABLE").isEmpty()) {
        sqlBuilder.append("FROM ").append(queryParams.get("TABLE")).append(" ");
    }
    
    // JOIN
    if (!queryParams.get("JOIN").isEmpty()) {
        sqlBuilder.append(queryParams.get("JOIN")).append(" ");
    }
    
    // WHERE
    if (!queryParams.get("WHERE").isEmpty()) {
        sqlBuilder.append("WHERE ").append(queryParams.get("WHERE")).append(" ");
    }
    
    // OTHER (GROUP BY, ORDER BY, etc.)
    if (!queryParams.get("OTHER").isEmpty()) {
        sqlBuilder.append(queryParams.get("OTHER"));
    }
